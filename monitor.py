...
def cg(path):
    ...
    res = get_json("https://api.coingecko.com/api/v3" + path, headers=headers)
    time.sleep(2.5 if CG_KEY else 8)
    return res
...
def render(ctx):
    ...
    if red_b["n"] < TH["verify_min_samples"]:
        verdict = f'<span class="warn-text">검증 전</span> (7일이 지난 강조 {red_b["n"]}개 / 필요 {TH["verify_min_samples"]}개)'
    else:
        verdict = f'강조 {red_b["n"]}개 중 {red_b["wins"]}개가 섹터보다 좋았음 ({red_b["rate"]:.0f}%)'
    props = f"""
<dl class="props">
...
<details class="sector">{head}
 ...
 <details class="inner"><summary>섹터 판정 근거</summary>
  ...
  <tr><td>펀딩 연환산</td><td>{f_pct(sec["fund_apr"])}</td></tr>{chain_kv}</table>
    ...
    board_html = f"""<div class="tbl"><table class="list"><tr><th>구분</th><th>누적</th><th>7일 경과</th><th>섹터보다 좋았던 비율</th><th>평균 초과 수익</th></tr>
{brow("지금 볼 것", red_b)}{brow("관심 목록", yel_b)}</table></div>
<p class="na">7일 경과 표본이 {TH["verify_min_samples"]}개를 넘고 비율이 55% 이상일 때까지는 이 도구를 진입 근거로 쓰지 마세요.</p>"""
...
def main():
    ...
    meta = {"obs": f"{int(days) + 1}일째 (권장 4~8주 관찰 후 사용)", "cat_age": cat_age, "empty_reason": empty}
